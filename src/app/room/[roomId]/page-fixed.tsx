      ...
      if (!isScreenSharing) {
        ...
        if (stream) {
          ...
          peersRef.current.forEach(({ peer }) => {
            const peerWithSenders = peer as PeerWithSenders;
            if (peerWithSenders && peerWithSenders._senders) {
              const sender = peerWithSenders._senders.find(s => s.track && s.track.kind === 'video');
              if (sender) {
                sender.replaceTrack(screenCaptureStream.getVideoTracks()[0]);
              }
            }
          });
          ...
          screenCaptureStream.getVideoTracks()[0].onended = () => {
            ...
            // Restore video for peers
            peersRef.current.forEach(({ peer }) => {
              const peerWithSenders = peer as PeerWithSenders;
              if (peerWithSenders && peerWithSenders._senders) {
                const sender = peerWithSenders._senders.find(s => s.track && s.track.kind === 'video');
                if (sender && videoTrack && videoTrack.readyState === 'live') {
